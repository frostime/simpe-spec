...
- [ ] Run `uv run sspec project update`
- [ ] Run `uv run pytest tests/test_change_service.py tests/test_change_command.py -q`
- [ ] Run `uv run ruff check src/`
- [ ] Sandbox-check `sspec change new/status` for both single and root changes under `tmp/`, plus one old-shape change status case
**Verification**: tests pass, lint passes, and self-hosted copies plus sandbox output reflect the repaired contract and explicit old-shape cutoff
...
| Phase 4: Self-host sync and verification | 0% | ⏳ |
