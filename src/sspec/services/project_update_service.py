...
from sspec.skill_installer import SkillInstaller, SkillStrategy, check_path_link, remove_path_link
...
def sync_hub_skills_gitignore(
    *, sspec_root: Path, managed_skill_names: list[str], dry_run: bool
    ...
    """Sync hub-managed skill ignores in `.sspec/.gitignore` and clean legacy hub-local blocks."""
    ...
    synced_root = SkillInstaller.sync_managed_gitignore_entries(
        sspec_root / '.gitignore',
        managed_entries,
        dry_run=dry_run,
    )
    cleaned_legacy = SkillInstaller.sync_managed_gitignore_entries(
        hub_skills_dir / '.gitignore',
        [],
        dry_run=dry_run,
    )
    return synced_root or cleaned_legacy
