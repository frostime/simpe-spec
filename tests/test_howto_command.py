...
def test_howto_list_uses_plain_text_default_output(tmp_path: Path, monkeypatch) -> None:
    ...
    assert 'name\tsource\tdescription\tfile' in result.output
    assert 'local-guide\tproject\tlocal test guide\tlocal-guide.md' in result.output
...
def test_howto_rich_format_uses_pretty_rendering(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.chdir(tmp_path)
    runner = CliRunner()
    result = runner.invoke(main, ['howto', '--format', 'rich', 'local-guide'])
